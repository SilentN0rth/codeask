...
export default function App({
  ...
  useEffect(() => {
    if (editorRef.current && isEditorReady) {
      const currentContent = (
        editorRef.current as unknown as { getContent: () => string }
      ).getContent();
  ...
  useEffect(() => {
    if (editorRef.current) {
      ...
      if (editorContainer) {
        ...
        const toolbarPrimary = editorContainer.querySelector(
  ...
  return (
    <div className="relative min-h-[100px]">
      ...
      <Editor
        onInit={(_, editor) => {
          // @ts-expect-error - Type mismatch with Editor types
          ...
          const fixAccessibility = () => {
            const resizeHandle = editor
              .getContainer()
              ?.querySelector('.tox-statusbar__resize-handle');
            if (resizeHandle) {
              resizeHandle.setAttribute('aria-valuenow', '400');
              resizeHandle.setAttribute('aria-valuemin', '100');
              resizeHandle.setAttribute('aria-valuemax', '800');
            }

            const iframe = editor.getContainer()?.querySelector('iframe');
            if (iframe) {
              iframe.setAttribute('title', 'Edytor tekstu sformatowanego');
            }
          };
        ...
        init={{
          ...
          quickbars_selection_toolbar:
            'bold italic underline | quicklink h2 h3 p pre blockquote codesample',
          menubar: false,
          toolbar:
