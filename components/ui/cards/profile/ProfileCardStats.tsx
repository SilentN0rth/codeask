...
export default function ProfileCardStats({
  ...
  const stats = [
    {
      ...
      label: 'Pytania',
      value: author?.questions_count,
    },
    {
      icon: 'mdi:comment-text-multiple',
      label: 'Odpowiedzi',
      value: author?.answers_count,
    },
  ...
  return (
    <CardBody
      className={`border-t border-divider bg-cBgDark-700 !p-0 ${isRounded ? 'rounded-b-lg' : ''}`}
    >
      <div className="grid grid-cols-3 text-center">
        {stats.map((stat) => (
          <div
            key={`stat-${stat.label}-${stat.value}`}
            className="cursor-default px-3 py-5 transition-colors hover:bg-cBgDark-800"
          >
