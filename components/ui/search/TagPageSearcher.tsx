...
const TAG_PAGE_FILTER_OPTIONS = [
  { label: 'Użytkownik', value: 'user' },
  { label: 'Status pytań', value: 'status' },
];

const TagPageSearcher = forwardRef<
  { resetAllFilters: () => void },
  ClassName & { users?: UserInterface[] }
>(({ className, users = [] }, ref) => {
  ...
  const renderFilterComponent = () => {
    ...
    switch (selectedFilter) {
      case 'user':
        return <UserFilter {...commonProps} users={users} />;
      case 'status':
        return <StatusFilter {...commonProps} />;
      default:
        return null;
    }
  };
  ...
  return (
    ...
    >
      ...
      <Select
        size="sm"
        radius="sm"
        className={`col-span-5 h-full ${isCompact ? 'sm:col-span-4 md:col-span-4 lg:col-span-2 xl:col-span-2' : 'lg:col-span-4 2xl:col-span-3'}`}
        label="Wybierz filtr"
      ...
      >
        {TAG_PAGE_FILTER_OPTIONS.map((option) => (
...
TagPageSearcher.displayName = 'TagPageSearcher';

export default TagPageSearcher;
