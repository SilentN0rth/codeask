...
const Text = ({ as: Component, children, className = '' }: TextProps) => {
  return (
    <Component className={`${className} break-words`}>{children}</Component>
  );
};
  ...
  return (
    <div className={`text-sm ${className} w-96`}>
      ...
      <Button
        ...
        type="button"
        radius="none"
        className={`block !h-fit !min-w-fit overflow-visible !bg-transparent !p-0 text-cCta-500 underline underline-offset-4 hover:underline ${isExpanded ? 'mt-1' : ''}`}
      >
