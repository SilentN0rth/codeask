...
export const TagItem: React.FC<TagItemProps> = ({
  ...
  const content = (
    <span
      className={`inline-flex items-center gap-1 rounded-md border border-divider text-sm text-cTextDark-100 transition-all hover:border-cCta-500/30 hover:bg-cCta-500/10 active:scale-95 ${className}`}
    >
      #{label}
      {onClose && (
        <button
