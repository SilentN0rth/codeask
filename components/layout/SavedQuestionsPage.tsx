...
import SavedQuestionsSearcher from '../ui/search/SavedQuestionsSearcher';
...
export default function SavedQuestionsPage({
  ...
  const QuestionCardWrapper = ({
    ...
    >
      <QuestionCard
        {...question}
        savedQuestionIds={questions.map((q) => q.id)}
      />
    </motion.div>
  );
  ...
  return (
    ...
    >
      <motion.div {...titleAnimation}>
        <div className="flex items-center justify-between">
          <PageTitle
            ...
            description="Pytania, które zapisałeś do późniejszego przeglądu"
          />
          <Button
            as={Link}
            href="/questions"
            ...
            aria-label="Przejdź do wszystkich pytań"
          >
            Wszystkie pytania
          </Button>
        </div>
      ...
      <motion.div {...searchAnimation}>
        <SavedQuestionsSearcher ref={searcherRef} className="mb-4" />
      </motion.div>
      ...
      {isLoading ? (
          ...
          <motion.div {...cardsAnimation}>
            {viewMode === 'masonry' && (
              <Masonry
                ...
                itemAs="li"
                role="list"
                aria-label="Lista zapisanych pytań w układzie masonry"
              />
            )}
          ...
          >
            {totalPages > 1 && (
              <UniversalPagination
                ...
                ariaLabel="Przejdź między stronami zapisanych pytań"
              />
            )}
          </motion.div>
        </>
        ...
        <NoResults
          title="Brak zapisanych pytań"
          description="Nie masz jeszcze żadnych zapisanych pytań."
          hint="Przeglądaj pytania i zapisuj te, które Cię interesują."
          icon="material-symbols:bookmark-outline"
        />
      )}
    </motion.div>
