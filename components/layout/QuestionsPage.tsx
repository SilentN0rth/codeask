...
import { useQuestionViewsRealtime } from '@/hooks/useQuestionViewsRealtime';
...
export default function QuestionsPage({
  ...
  const realtimeQuestions = useQuestionViewsRealtime(questions);
  ...
  const hasActiveFilters = Boolean(
    currentSearch || currentFilter || currentValue
  );
    ...
    >
      <motion.div {...titleAnimation}>
        <PageTitle
          ...
          description="Przeglądaj wszystkie pytania"
          iconClasses=""
          parentClasses="mb-6 flex-col md:flex-row justify-between"
          innerClasses=" flex gap-x-3"
        >
          <Button
            onPress={() => {
              if (!user) {
            ...
            variant="light"
            radius="md"
            className="my-auto bg-cCta-500 text-cTextDark-100 transition-colors hover:!bg-cCta-700"
          >
            Dodaj pytanie
          </Button>
        </PageTitle>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {realtimeQuestions.length === 0
                ? 'wyników'
                : realtimeQuestions.length === 1
                  ? 'wynik'
                  : realtimeQuestions.length < 5
                    ? 'wyniki'
                    : 'wyników'}
            </Chip>
            <ClearFiltersButton
      ...
      {isLoading ? (
        ...
        <>
          ...
          {totalPages > 1 && (
            <motion.div {...paginationAnimation} className="mt-4">
              <UniversalPagination
                currentPage={currentPage}
                totalPages={totalPages}
                onPageChange={setPage}
                ariaLabel="Przejdź między stronami pytań"
              />
            </motion.div>
          )}
        </>
        ...
        <NoResults
          title="Brak pytań"
          description="Nie znaleziono pytań spełniających kryteria wyszukiwania."
