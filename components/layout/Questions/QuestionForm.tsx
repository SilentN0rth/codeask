...
export default function QuestionForm({ userId }: { userId: string }) {
    ...
    const parseAndAddTags = async (): Promise<{ success: boolean; wasModified: boolean }> => {
    ...
    const onSubmit = handleSubmit(async (data) => {
        const success = await parseAndAddTags();
        if (!success) return;
