...
const editProfileSchema = z.object({
    ...
    username: z
        ...
        .regex(/^[a-zA-Z0-9_]+$/, "Dozwolone są tylko litery, cyfry i podkreślenie"),
    ...
    location: z.string().max(50, "Lokalizacja może mieć maksymalnie 50 znaków").optional(),
    ...
    specialization: z.string().max(30, "Specjalizacja może mieć maksymalnie 30 znaków").optional(),
});
...
type Props = {
    ...
    onSubmit: (data: UserEditFormData, onClose: () => void) => void;
    onClose: () => void;
};

export default function EditProfileForm({ defaultValues, onSubmit, onClose }: Props) {
    const {
        control,
        handleSubmit,
        formState: { errors },
...
border border-divider bg-cBgDark-800 p-6 pb-5 pt-4 shadow-none xl:col-span-1">
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            <Input
                                ...
                                classNames={{ inputWrapper: "bg-cBgDark-700 border border-divider" }}
                            />
                        )}
                    />
                    <Controller
